...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info("Generating image with Gemini for prompt:", body.prompt);
    logger.debug("Page number:", body.pageNumber || "N/A");
    logger.debug("Has reference image:", !!body.referenceImage);
    ...
    if (body.referenceImage && body.pageNumber && body.pageNumber > 1) {
      enhancedPrompt +=
        ". IMPORTANT: Keep the character design, art style, and visual consistency exactly the same as in the reference image. Maintain the same character appearance, color palette, and artistic style.";
    }
    ...
    if (!apiKey) {
      ...
      return NextResponse.json(
        {
          error:
            "Google Generative AI API key is required. Please add GOOGLE_GENERATIVE_AI_API_KEY to your environment variables.",
        },
        { status: 500 },
      );
    }
    ...
    if (body.referenceImage && body.pageNumber && body.pageNumber > 1) {
      ...
      logger.debug("Using reference image for consistency");
      contents = [
        { text: enhancedPrompt },
        {
          inlineData: {
      ...
      logger.debug("Generating first image (text-to-image)");
